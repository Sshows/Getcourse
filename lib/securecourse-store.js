...
const VERIFICATION_CODE_MINUTES = Number(process.env.SECURECOURSE_VERIFICATION_CODE_MINUTES || 15);
...
function maskPhone(value) {
  ...
  return `${phone.slice(0, Math.max(phone.length - 8, 2))}${"*".repeat(4)}${tail}`;
}
...
function reconcileState(state) {
  ...
  state.verificationChallenges.forEach((challenge) => {
    if (challenge.status === "PENDING" && new Date(challenge.expiresAt).getTime() <= timestamp) {
...
function getStudentAccountById(state, studentAccountId) {
  return state.studentAccounts.find((account) => account.id === studentAccountId) || null;
}

function getStudentAccountByLogin(state, login) {
  const normalizedEmail = String(login || "").trim().toLowerCase();
  const normalizedPhone = normalizePhone(login);

  return (
    state.studentAccounts.find(
      (account) => account.email === normalizedEmail || (normalizedPhone && account.phone === normalizedPhone)
    ) || null
  );
}

function buildStudentAccountSummary(account) {
  if (!account) {
    return null;
  }
  ...
  return {
    ...
    fullyVerified: Boolean(account.emailVerifiedAt && account.phoneVerifiedAt)
  };
}

function buildStudentAccountView(state, account) {
  const user = getUserById(state, account.storeUserId);

  return {
    id: account.id,
    storeUserId: account.storeUserId,
    email: account.email,
    phone: account.phone,
...
function ensureStudentUserRecord(state, payload) {
  ...
  if (user.role !== "STUDENT") {
    throw new SecureCourseError(409, "This email is already reserved for another account.");
  }
...
function createStudentDirectSession(state, user, metadata = {}) {
  revokeActiveStudentSessions(state, user.id, metadata.replaceReason || "student_password_login");
  ...
  const session = {
    ...
    accessTokenId: null,
    status: "ACTIVE",
    deviceId: metadata.deviceId || "securecourse-web-password",
    deviceFingerprint: metadata.deviceFingerprint || "securecourse-web-password",
    deviceLabel: metadata.deviceLabel || "SecureCourse Website",
...
function issueVerificationChallenge(state, account, channel) {
  const timestamp = nowIso();
  ...
  state.verificationChallenges
    .filter(
      (challenge) =>
        challenge.studentAccountId === account.id && challenge.channel === channel && challenge.status === "PENDING"
    )
    .forEach((challenge) => {
      challenge.status = "EXPIRED";
  ...
  const challenge = {
    ...
    createdAt: timestamp,
    updatedAt: timestamp,
    expiresAt: futureIso(VERIFICATION_CODE_MINUTES),
    consumedAt: null
  };

  state.verificationChallenges.push(challenge);
...
function consumeVerificationChallenge(state, account, channel, rawCode) {
  const code = String(rawCode || "").trim();

  if (!code) {
    throw new SecureCourseError(400, `${channel} verification code is required.`);
  }

  const challenge = state.verificationChallenges
    .filter(
      (item) => item.studentAccountId === account.id && item.channel === channel && item.status === "PENDING"
    )
    .sort((left, right) => right.createdAt.localeCompare(left.createdAt))[0];

  if (!challenge) {
    throw new SecureCourseError(404, `No active ${channel.toLowerCase()} verification challenge found.`);
  }
  ...
  if (new Date(challenge.expiresAt).getTime() <= Date.now()) {
    ...
    challenge.updatedAt = nowIso();
    persistState(state);
    throw new SecureCourseError(401, `${channel} verification code expired.`);
  }

  challenge.attemptCount = Number(challenge.attemptCount || 0) + 1;
  challenge.updatedAt = nowIso();

  if (challenge.codeHash !== hashVerificationCode(code)) {
    if (challenge.attemptCount >= 5) {
      challenge.status = "EXPIRED";
    }
    persistState(state);
    throw new SecureCourseError(401, `${channel} verification code is invalid.`);
  }
...
function buildUserView(state, user) {
  ...
  const studentAccount = state.studentAccounts.find((account) => account.storeUserId === user.id) || null;

  return {
    ...user,
    enrollments,
    studentAccount: buildStudentAccountSummary(studentAccount)
  };
}
...
export function createUser(adminSessionId, payload) {
  ...
  const linkedAccount = state.studentAccounts.find((account) => account.email === email) || null;
  if (linkedAccount) {
    linkedAccount.storeUserId = user.id;
    linkedAccount.fullName = fullName;
    if (phone) {
      linkedAccount.phone = phone;
    }
    linkedAccount.updatedAt = timestamp;
  }
...
export function registerStudentAccount(payload, metadata = {}) {
  const state = loadState();
  const fullName = String(payload.fullName || "").trim();
  const email = String(payload.email || "").trim().toLowerCase();
  const phone = normalizePhone(payload.phone);
  ...
  if (!fullName || !email || !phone || !password) {
    throw new SecureCourseError(400, "Full name, email, phone, and password are required.");
  }

  if (password.length < 8) {
    throw new SecureCourseError(400, "Password must be at least 8 characters long.");
  }

  if (state.studentAccounts.some((account) => account.email === email)) {
    throw new SecureCourseError(409, "Student account with this email already exists.");
  }

  if (state.studentAccounts.some((account) => account.phone === phone)) {
    throw new SecureCourseError(409, "Student account with this phone number already exists.");
  }

  const user = ensureStudentUserRecord(state, { fullName, email, phone });
  const timestamp = nowIso();
  const account = {
    id: randomUUID(),
    storeUserId: user.id,
  ...
  state.studentAccounts.push(account);

  const emailChallenge = issueVerificationChallenge(state, account, "EMAIL");
  const smsChallenge = issueVerificationChallenge(state, account, "SMS");

  recordAudit(state, {
    actorId: user.id,
    actorType: "USER",
    eventType: "STUDENT_REGISTRATION_STARTED",
  ...
  return {
    student: buildStudentAccountView(state, account),
    verification: {
      email: {
        destination: email,
        maskedDestination: emailChallenge.maskedDestination,
        code: emailChallenge.code,
        expiresAt: emailChallenge.challenge.expiresAt
      },
      sms: {
        destination: phone,
        maskedDestination: smsChallenge.maskedDestination,
        code: smsChallenge.code,
        expiresAt: smsChallenge.challenge.expiresAt
      }
    }
  };
}

export function resendStudentVerification(payload, metadata = {}) {
  const state = loadState();
  const account = getStudentAccountByLogin(state, payload.email || payload.phone || payload.login);

  if (!account) {
    throw new SecureCourseError(404, "Student account not found.");
  }

  const emailChallenge = account.emailVerifiedAt ? null : issueVerificationChallenge(state, account, "EMAIL");
  const smsChallenge = account.phoneVerifiedAt ? null : issueVerificationChallenge(state, account, "SMS");

  recordAudit(state, {
    actorId: account.storeUserId,
    actorType: "USER",
    eventType: "STUDENT_VERIFICATION_RESENT",
    entityType: "student_account",
    entityId: account.id,
    metadata: {
      ipAddress: metadata.ipAddress || null
...
export function verifyStudentAccount(payload, metadata = {}) {
  const state = loadState();
  const account = getStudentAccountByLogin(state, payload.email || payload.phone || payload.login);
  ...
  if (!account.emailVerifiedAt) {
    consumeVerificationChallenge(state, account, "EMAIL", payload.emailCode);
  ...
  if (!account.phoneVerifiedAt) {
    consumeVerificationChallenge(state, account, "SMS", payload.smsCode);
    account.phoneVerifiedAt = nowIso();
    account.updatedAt = nowIso();
    persistState(state);
  }

  if (!account.emailVerifiedAt || !account.phoneVerifiedAt) {
    throw new SecureCourseError(400, "Email and phone verification are both required.");
  }
  ...
  if (!user || user.status !== "ACTIVE") {
    throw new SecureCourseError(403, "Student profile is not active.");
  }

  const { session, user: studentUser } = createStudentDirectSession(state, user, {
    ...metadata,
    replaceReason: "verified_student_login"
  });
...
export function loginStudentWithPassword(payload, metadata = {}) {
  const state = loadState();
  const account = getStudentAccountByLogin(state, payload.login || payload.email || payload.phone);
  const password = String(payload.password || "");

  if (!account) {
    throw new SecureCourseError(401, "Student account not found.");
  }

  if (!verifyPassword(password, account.passwordHash)) {
    throw new SecureCourseError(401, "Password is invalid.");
  }

  if (!account.emailVerifiedAt || !account.phoneVerifiedAt) {
    throw new SecureCourseError(403, "Email and phone verification must be completed first.");
  }

  if (account.status === "BLOCKED") {
    throw new SecureCourseError(403, "Student account is blocked.");
  }
  ...
  const { session, user: studentUser } = createStudentDirectSession(state, user, {
    ...metadata,
    replaceReason: "student_password_login"
  });
...
export function getStudentSessionView(userId, sessionId) {
  ...
  try {
    const { user, session } = ensureStudentSession(state, userId, sessionId, { touch: false });
    return {
      authenticated: true,
      user: {
        id: user.id,
