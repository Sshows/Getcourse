...
export const SECURECOURSE_ADMIN_SESSION_HEADER = "x-securecourse-admin-session";
export const SECURECOURSE_USER_HEADER = "x-securecourse-user-id";
export const SECURECOURSE_SESSION_HEADER = "x-securecourse-session-id";
...
export function readSecureCourseSession(request) {
  const userId =
    readCookieValue(request, SECURECOURSE_USER_COOKIE) || request?.headers?.get?.(SECURECOURSE_USER_HEADER);
  const sessionId =
    readCookieValue(request, SECURECOURSE_SESSION_COOKIE) ||
    request?.headers?.get?.(SECURECOURSE_SESSION_HEADER);
...
export function readSecureCourseAdminSession(request) {
  const sessionId =
    readCookieValue(request, SECURECOURSE_ADMIN_SESSION_COOKIE) ||
    request?.headers?.get?.(SECURECOURSE_ADMIN_SESSION_HEADER);
